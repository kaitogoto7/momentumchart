...
```{r}
win.record.round <- function(nation, round){
  filename <- paste0("./data/", nation, ".csv")
  result.vector.old.to.new <- read.csv(filename)[,-c(1)]
  result.matrix.old.to.new <- matrix(result.vector.old.to.new, ncol = 5, byrow = TRUE)[,round] #round on each column, year on each row with oldest at top (ie row 1)
  
  result.vector.old.to.new <- as.vector(result.matrix.old.to.new)
  win.record <- sum(result.vector.old.to.new == "won")/length(result.vector.old.to.new)
  
  return(win.record)
}

win.record.round("England", 1)
```
...
trace1 <- list(
  x = nations, 
  y = c(win.record.round("England", 1), win.record.round("France", 1), win.record.round("Ireland", 1), win.record.round("Italy", 1), win.record.round("Scotland", 1), win.record.round("Wales", 1)), #each of the nations' win record when they lose round 1 from England to Wales
  marker = list(
    #color = "gray", 
...
trace2 <- list(
  x = nations, 
  y = c(win.record.round("England", 2), win.record.round("France", 2), win.record.round("Ireland", 2), win.record.round("Italy", 2), win.record.round("Scotland", 2), win.record.round("Wales", 2)), 
  marker = list(
    #color = "darkorange", 
    line = list(color = "transparent")
  ), 
...
trace3 <- list(
  x = nations, 
  y = c(win.record.round("England", 3), win.record.round("France", 3), win.record.round("Ireland", 3), win.record.round("Italy", 3), win.record.round("Scotland", 3), win.record.round("Wales", 3)), 
  marker = list(
    #color = "gray", 
    line = list(color = "transparent")
  ), 
...
trace4 <- list(
  x = nations, 
  y = c(win.record.round("England", 4), win.record.round("France", 4), win.record.round("Ireland", 4), win.record.round("Italy", 4), win.record.round("Scotland", 4), win.record.round("Wales", 4)),
  marker = list(
    #color = "darkorange", 
    line = list(color = "transparent")
  ), 
...
trace5 <- list(
  x = nations, 
  y = c(win.record.round("England", 5), win.record.round("France", 5), win.record.round("Ireland", 5), win.record.round("Italy", 5), win.record.round("Scotland", 5), win.record.round("Wales", 5)), 
  marker = list(
    #color = "gray", 
    line = list(color = "transparent")
  ), 
...
layout <- list(
  ...
  showlegend = FALSE, 
  title = list(text = "6N WIN RECORD FOR EACH ROUND"), 
  width = 750, 
  xaxis = list(
    autorange = TRUE, 
...
api_create(p, filename = "Six Nations Round Records")
```